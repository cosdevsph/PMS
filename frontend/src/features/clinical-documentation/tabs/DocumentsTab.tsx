...
export const DocumentsTab = () => {
  ...
  const filteredDocs = documents.filter(doc => {
    ...
    const matchesCase = selectedCaseId ? doc.patient_case === selectedCaseId : false;
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50/50">
        ...
        {!selectedCaseId ? (
          <div className="bg-white rounded-xl border border-slate-200 p-8 text-center shadow-sm mt-10">
            ...
            <p className="text-sm text-slate-500 mt-1">
              Please select a case from the dropdown above to view documents.
            </p>
          </div>
        ) : isLoading ? (
      ...
      {isUploadModalOpen && selectedCaseId && (
        <UploadDocumentModal
